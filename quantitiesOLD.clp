...
:- pick(A,O,S,T), &sum{ amountOnShelf(A,S,T-1); -amountPicked(A,O,S,T) } < 0.

:-     pick(A,O,S,T), onShelf(A,S),               &sum{ amountOnShelf(A,S,T-1); -amountOnShelf(A,S,T); -amountPicked(A,O,S,T) } != 0.
:-     pick(A,O,S,T),               ordered(O,A), &sum{ amountMissing(A,O,T-1); -amountMissing(A,O,T); -amountPicked(A,O,S,T) } != 0.
:-     pick(A,O,S,T),                             &sum{                                                 amountPicked(A,O,S,T) }  = 0.
